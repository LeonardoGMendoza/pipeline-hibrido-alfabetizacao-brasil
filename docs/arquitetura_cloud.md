...
- *Processamento Batch*: AWS EMR e AWS Glue rodando nossos scripts PySpark.
...
- *Bronze*: Dados brutos do INEP formatados (CSV->Parquet).
- *Silver*: Realizados JOINs entre as tabelas de Alunos, Municípios e Metas.
- *Gold*: Regras de negócio de alfabetização (Nota >= 743 pts), consolidação final.
